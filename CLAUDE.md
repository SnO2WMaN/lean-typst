...
A Lean 4 project that adds a `#typst` command for compiling and displaying Typst
snippets as images in the Lean Infoview, rendered client-side via a WASM build of
...
- `LeanTypst.lean` / `LeanTypst/` is the library root; `LeanTypst/Typst.lean` defines the `#typst "..."` command.
- `Main.lean` is the executable entry point and also hosts the `#typst` demos (open it in VS Code to see them render in the Infoview).
