...
- `LeanTypst/Typst.lean`: the `#typst` elaborator saves a `LeanTypst.TypstDisplay` panel widget with the raw Typst source string as props (`include_str`-embeds the compiled `widget/dist/typstDisplay.js`); actual *rendering* always happens later, client-side, when the infoview renders the widget. Before that, though, `checkTypstCompiles` best-effort-compiles the source with a local `typst` binary (via `IO.Process.output`, piping `src` to `typst compile - -f svg -` and discarding the SVG) if one is on `PATH`, and `throwErrorAt`s the command with the compiler's diagnostic on failure — this turns a broken/blank widget (only noticeable by eye in the infoview) into a real Lean error surfaced through normal diagnostics (`lake build`, editor squiggly, etc.). If `typst` isn't installed, this check is silently skipped and the command behaves as before (rendering, and any errors, are then entirely client-side/WASM); it is not a hard requirement.
